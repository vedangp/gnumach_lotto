...
struct kmem_cache lotto_ticket_zone;
struct kmem_cache lotto_currency_zone;
/*
  ...
  (lotto_ticket_t ticket)
{
  ...
  printf("lotto_ticket_activate: active tickets %d\n",ticket->currency->amount);
#endif
  ...
  (lotto_ticket_t ticket)
{
  ...
  {
    printf("currency will be deactvated %d\n", ticket->currency->amount);
...
void create_user_and_system_currency( processor_set_t  pset);
  ...
  (void)
{
  ...
  /* initialize storage zones */
    ...
    create_user_and_system_currency(&default_pset);	
...
void create_user_and_system_currency( processor_set_t pset)
{
	...
	printf("Lottery Scheduling: creating base currency.\n");
	...
	printf("Lottery Scheduling: creating system currency.\n");
	/* create and fund system currency */
	ret1 = lotto_prim_create_currency(pset,
					  LOTTO_CURRENCY_NAME_SYSTEM, 
					  LOTTO_CURRENCY_ID_SYSTEM,
					  &pset->lotto_system_currency,
					  &currency);
	ret2 = lotto_prim_create_ticket(pset,
					&pset->lotto_base_currency,
	...
	printf("Lottery Scheduling: creating user currency.\n");
   ...
   boolean_t idle)
{
  ...
  do
	...
	break;
      ...
      if (current_runnable)
	{
	  printf("RUN STATE : %d , PROCESSOR: %d, POLICY: %d curr amount: %d curr act %d\n", 
		current->state,
		current->bound_processor,
		current->policy,
		current->lotto_thread_currency->amount,
		current->lotto_thread_currency->activated); 
	  /* update running sum */
...
struct kmem_cache lotto_ipc_xfer_zone;
